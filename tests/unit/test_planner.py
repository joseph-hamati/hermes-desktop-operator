...
async def test_malformed_model_output(settings, monkeypatch) -> None:
    ...
    planner = OllamaPlanner(settings)
    with pytest.raises(PlannerError, match="invalid actions after retry"):
        await planner.plan("open notepad")


@pytest.mark.asyncio
async def test_model_output_rejects_unknown_action_fields(settings, monkeypatch) -> None:
    async def fake_post(self, url, json):  # noqa: ANN001
        payload = {"actions": [{"type": "wait", "duration": 1}]}
        return httpx.Response(
            200,
